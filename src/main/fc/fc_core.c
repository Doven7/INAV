...
static void emergencyArmingReset(void)
{
    memset(&emergencyArming, 0, sizeof(emergencyArming));
}
...
void emergencyArmingUpdate(bool armingSwitchIsOn)
{
    ...
    if (armingSwitchIsOn) {
        ...
        if (emergencyArming.queueCount == 0) {
            ...
            while (emergencyArming.start < now - EMERGENCY_ARMING_TIME_WINDOW_MS) {
